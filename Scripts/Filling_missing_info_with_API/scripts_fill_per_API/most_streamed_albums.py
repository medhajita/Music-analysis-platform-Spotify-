...
INPUT_CSV = "D:/ECE_WORK/Music-analysis-platform-Spotify-/Scripts/Filling_missing_info_with_API/new_csv/most_streamed_albums_enriched.csv"
PROGRESS_CSV = "D:/ECE_WORK/Music-analysis-platform-Spotify-/Scripts/Filling_missing_info_with_API/NEW_API_CSV/progress_most_streamed_albums.csv"
SAVE_EVERY = 25
...
def enrich_csv():
    ...
    unique_artists = df[df["country"] == "TBD"]["artist"].unique().tolist()
    ...
    for index, row in df.iterrows():
        if row["country"] == "TBD":
            info = progress_dict.get(row["artist"])
            if info:
