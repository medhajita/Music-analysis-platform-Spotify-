...
df["date_peak_listeners"] = pd.to_datetime(
    df["date_peak_listeners"],
    format="%d.%m.%y",
    errors="coerce"          # NaN si la valeur ne parse pas, pas d'exception
).dt.strftime("%Y-%m-%d")   # NaN reste NaN, pas de crash
